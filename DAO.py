...
class DAO:
    ...
    def readData(self):
        ...
        cars_test["category"] = cars_test["category"].fillna("car")
    ...
    def normalise(self):
        ...
        norm["highway_mpg_norm"] = (norm["highway_mpg"] - norm["highway_mpg"].min()) / (
                    norm["highway_mpg"].max() - norm["highway_mpg"].min())
        norm["city_mpg_norm"] = (norm["city_mpg"] - norm["city_mpg"].min()) / (
                    norm["city_mpg"].max() - norm["city_mpg"].min())
        norm["popularity_norm"] = (norm["popularity"] - norm["popularity"].min()) / (
                    norm["popularity"].max() - norm["popularity"].min())
        ...
        norm["cylinders_norm"] = (norm["cylinders"] - norm["cylinders"].min()) / (
                    norm["cylinders"].max() - norm["cylinders"].min())
        ...
        for key, value in search_terms.items():
            ...
            if str(value).lower() != "any":
                ...
                # matching_cars = matching_cars[matching_cars[key] >= int(value)] if user wants efficiency logic shouldn't provide more cylinders
                    ...
                    matching_cars = matching_cars[matching_cars[key].apply(
                        lambda x: len(set(str(x).lower().split(";")).intersection(set(accepted))) > 0)]
                else:
                    print(key, value)
                    matching_cars = matching_cars[
    ...
    def showCarInfo(self, car):
        ...
        car_info = f"Make: {make}\n"
        car_info += f"Model: {model}\n"
        car_info += f"Year: {year}\n"
        car_info += f"Horsepower: {hp}\n"
        car_info += f"Cylinders: {cylinders}\n"
        car_info += f"Doors: {doors}\n"
        car_info += f"Category: {category}\n"
        car_info += f"Highway MPG: {highway_mpg}\n"
        car_info += f"City MPG: {city_mpg}\n"
        car_info += f"Popularity: {popularity}\n"
        car_info += f"Price: ${price}\n"
        car_info += f"Fuel Type: {fuel_type}\n"
        car_info += f"Transmission: {transmission}\n"
        car_info += f"Driven Wheels: {driven_wheels}\n"
        car_info += f"Size: {size}\n"
        car_info += f"Style: {style}\n"
    ...
    def dataAnalysis(self):
        ...
        data = data.drop(
            columns=["make", "model", "category", "fuel_type", "transmission", "driven_wheels", "size", "style",
                     "index", "price_norm", "year_norm", "hp_norm", "highway_mpg_norm", "city_mpg_norm",
                     "popularity_norm", "doors_norm", "cylinders_norm"])
